import cv2
import numpy as np
from pitch_config_Soccana import PITCH_KEYPOINTS
...
class HomographyCalculator:
    ...
    def transform_point(self, point, H, canvas_w=None, canvas_h=None, margin=20):
        """Transform satu titik dengan H. Return None kalau H is None atau hasil out of range."""
        ...
        # Tolak koordinat yang tidak masuk akal (overflow homography)
        if abs(x) > 10000 or abs(y) > 10000:
            return None
        
        # Kalau canvas size diberikan, filter ketat
        if canvas_w is not None and canvas_h is not None:
            if not (-margin < x < canvas_w + margin and 
