...
class Tracker:
    ...
    #         if detections is not None and len(detections) > 0:
    #             # Only remap if class_ids appear to be 1-based (contain values > len(class_names)-1)
    #             max_id = int(detections.class_id.max())
    #             if max_id >= len(class_names):
    #                 # 1-based → 0-based, clamp to valid range
    #                 new_ids = np.clip(detections.class_id.astype(int) - 1, 0, len(class_names) - 1)
    ...
    def detect_frames(self, frames):
        ...
        for frame in frames:
            ...
            if detections is not None and len(detections) > 0:
                ...
                # ── Strip mismatched data fields (e.g. feature maps sized 1080) ──
                clean_data = {}
    ...
    def get_object_tracks(self, frames, read_from_stub=False, stub_path=None):
        ...
        for frame_num, detections in enumerate(detections_list):
            ...
            # Guard: ensure class_id length matches detection count
